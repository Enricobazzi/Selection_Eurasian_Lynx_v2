---
title: "Downstream_Analyses_finalset"
author: "Enrico"
date: "1/10/2022"
...
---
This version of downstream analysis will only analyze results using the "finalset" of samples instead of all of them. This means that I will not analyze European bottleneck populations (Carpathians, Poland, Norway, Balkans) and two Mongolian samples from Omnigovi.
...
bedtools intersect -header -a VCF/ll_wholegenome_LyCa_ref.sorted.filter7.finalset.maf5pc.vcf \
 -b Intersect/total_intersect_candidate_windows_topsnps.bed \
 > VCF/ll_finalset_total_intersect_candidate_windows_topsnps.vcf
...
vcftools --vcf ll_finalset_total_intersect_candidate_windows_topsnps.vcf --012 \
 --out finalset_topsnps_rformat

# edit outputs to create a table
cut -f2- finalset_topsnps_rformat.012 | sed 's/-1/NA/g' >finalset_topsnps_rformat.temp
tr -d '\t' <finalset_topsnps_rformat.012.pos | tr '\n' '\t' | sed 's/[[:space:]]*$//' >header
paste <(echo "ID" | cat - finalset_topsnps_rformat.012.indv) \
 <(echo "" | cat header - finalset_topsnps_rformat.temp) > finalset_topsnps_rformat.forR
rm header finalset_topsnps_rformat.temp
```
Download to laptop
```{bash}
scp ebazzicalupo@genomics-a.ebd.csic.es:/home/ebazzicalupo/Selection_Eurasian_Lynx/VCF/finalset_topsnps_rformat.forR Documents/Selection_Eurasian_Lynx_v2/4-Downstream_Analyses/tables/
```
Prepare R
```{R}
# load libraries
...
# SNP data
snp <- read.table("4-Downstream_Analyses/tables/finalset_topsnps_rformat.forR", header = T, row.names = 1)
snp <- snp[!(row.names(snp) %in% samples_to_remove),]

# env.data
clim.data <- read_tsv("2-Prepare_Environmental_Data/uncorrelated_variables_matrix_finalset.tsv", col_names = T) %>%
...
# coordinates
coord_table <- read_delim("~/Dropbox/LL_LC_LR_Databases/LL_coords/csv_LL_selection_coords_finalset.csv",
...
barplot(sort(gdm[[1]], decreasing=T))
gdm$explained
...
pdf(file = paste0("4-Downstream_Analyses/plots/GDM_finalset_effect_uncertainty.pdf"),
    width = 8,
    height = 8)
...
pdf("4-Downstream_Analyses/plots/GDM_finalset_Rbio2_Gbio6_BsnowD_RGBmap.pdf")
plotRGB(clim.trans, r=3, g=4, b=7, bgalpha=0, interpolate=TRUE)
points(clim.points$x,clim.points$y, pch=4, cex=0.2)
dev.off()
...
pca.rast[[4]] <- (pca.rast[[3]]-pca.rast[[3]]@data@min) / (pca.rast[[3]]@data@max-pca.rast[[3]]@data@min)*255
pca.rast[[5]] <- (pca.rast[[5]]-pca.rast[[5]]@data@min) / (pca.rast[[5]]@data@max-pca.rast[[5]]@data@min)*255

pdf("4-Downstream_Analyses/plots/GDM_finalset_pc123_RGBmap.pdf")
...
rm Intersect/bio2_bio6_snowdays_topsnps.range
for var in bio2 bio6 snow_days
 do
  echo "${var}"
  cat GenWin/${var}_topsnps.range >> Intersect/bio2_bio6_snowdays_topsnps.range
done
```
...
bedtools intersect -wo -a total_intersect_candidate_windows.bed \
 -b <(cat bio2_bio6_snowdays_topsnps.range | sort -k 1,1 -k2,2n) |
 cut -f1,2,3 | uniq -u > bio2_bio6_snowdays_topsnps_nodupwindows.bed
 
# get topsnp from each window with ONE topsnp
bedtools intersect -a <(cat bio2_bio6_snowdays_topsnps.range | sort -k 1,1 -k2,2n) \
 -b topsnps_nodupwindows.bed > bio2_bio6_snowdays_topsnps_nodupwindows_onesnp.bed

# get list of candidate windows with MORE than one topsnp
bedtools intersect -wo -a total_intersect_candidate_windows.bed \
 -b <(cat bio2_bio6_snowdays_topsnps.range | sort -k 1,1 -k2,2n) |
 cut -f1,2,3 | uniq -d > bio2_bio6_snowdays_topsnps_dupwindows.bed

# get only one topsnp for each of the windows with duplicates
while read p; do
  bedtools intersect -a <(cat bio2_bio6_snowdays_topsnps.range | sort -k 1,1 -k2,2n) -b <(echo "$p") | shuf -n 1
done < bio2_bio6_snowdays_topsnps_dupwindows.bed > bio2_bio6_snowdays_topsnps_dupwindows_onerand.bed

# get full list of topsnps (only one per window)
cat bio2_bio6_snowdays_topsnps_nodupwindows_onesnp.bed bio2_bio6_snowdays_topsnps_dupwindows_onerand.bed | 
 sort -k 1,1 -k2,2n \
 > bio2_bio6_snowdays_intersect_candidate_windows_topsnps.bed
```
...
bedtools intersect -header -a VCF/ll_wholegenome_LyCa_ref.sorted.filter7.finalset.maf5pc.vcf \
 ...
 > VCF/ll_finalset_bio2_bio6_snowdays_intersect_candidate_windows_topsnps.vcf
...
plink_1.9 --vcf ll_finalset_bio2_bio6_snowdays_intersect_candidate_windows_topsnps.vcf \
 --double-id --allow-extra-chr --set-missing-var-ids @:# \
 --remove samplestoremove.txt --geno 0 \
 --recode A --out finalset_bio2_bio6_snowdays_intersect_candidate_windows_topsnps
```
Copy to laptop
```{bash}
scp ebazzicalupo@genomics-a.ebd.csic.es:/home/ebazzicalupo/Selection_Eurasian_Lynx/VCF/finalset_bio2_bio6_snowdays_intersect_candidate_windows_topsnps.raw Documents/Selection_Eurasian_Lynx_v2/4-Downstream_Analyses/tables/
```
...
env.predictors <- read_tsv("2-Prepare_Environmental_Data/uncorrelated_variables_matrix_finalset.tsv", col_names = T) %>%
  column_to_rownames(., var="sample")
env.predictors <- env.predictors[, c('bio2', 'bio6', 'snow_days')]

# Load GenoType Data - choose set of vars file
gt_data <- read.PLINK("4-Downstream_Analyses/tables/finalset_bio2_bio6_snowdays_intersect_candidate_windows_topsnps.raw")
...
```{R}
# RDA1 v RDA2
pdf(file = paste0("4-Downstream_Analyses/plots/candidate_finalset_rda1_rda2.pdf"),
    width = 8,
    height = 8)
...
# RDA1 v RDA3
pdf(file = paste0("4-Downstream_Analyses/plots/candidate_finalset_rda1_rda3.pdf"),
    width = 8,
    height = 8)
...
Now to create a RAW file with plink with only 10k uncorrelated intergenic non-selected SNPs
```{bash}
cd /home/ebazzicalupo/Selection_Eurasian_Lynx/VCF/
# I manually created a file listing samples to remove for plink with following format:
# c_ll_ba_0216 c_ll_ba_0216 0 0 0 -9
...
# First intersect the wholeset intergenic no selection VCF with the finalset VCF
bedtools intersect -header -a ll_wholegenome_LyCa_ref.sorted.filter7.finalset.maf5pc.vcf \
 -b ll_wholegenome_LyCa_ref.sorted.filter7.intergenic.noselection.vcf \
 > ll_wholegenome_LyCa_ref.sorted.filter7.finalset.maf5pc.intergenic.noselection.vcf

# prune snps based on ld (VIF < 2)
plink_1.9 --vcf ll_wholegenome_LyCa_ref.sorted.filter7.finalset.maf5pc.intergenic.noselection.vcf \
--double-id --allow-extra-chr --set-missing-var-ids @:# --geno 0 \
--remove samplestoremove.txt --indep 100 10 2

# take only a subsample of 10k of the pruned SNPs but from the finalset vcf
plink_1.9 --vcf ll_wholegenome_LyCa_ref.sorted.filter7.finalset.maf5pc.intergenic.noselection.vcf \
--double-id --allow-extra-chr --set-missing-var-ids @:# \
--remove samplestoremove.txt --geno 0 --extract <(shuf -n 10000 plink.prune.in) \
--recode A --out finalset_intergenic_neutral_snps
```
Copy to laptop
```{bash}
scp ebazzicalupo@genomics-a.ebd.csic.es:/home/ebazzicalupo/Selection_Eurasian_Lynx/VCF/finalset_intergenic_neutral_snps.raw Documents/Selection_Eurasian_Lynx_v2/4-Downstream_Analyses/tables/
```
Prepare R for RDA on laptop
```{R}
# load libraries
...
# Load GenoType Data - choose set of vars file
gt_data <- read.PLINK("4-Downstream_Analyses/tables/finalset_intergenic_neutral_snps.raw")
gt_data_tsv <- data.frame(as.matrix(gt_data))
```
Run RDA
```{R}
...
```{R}
# RDA1 v RDA2
pdf(file = paste0("4-Downstream_Analyses/plots/neutral_finalset_rda1_rda2.pdf"),
    width = 8,
    height = 8)
...
# RDA1 v RDA3
pdf(file = paste0("4-Downstream_Analyses/plots/neutral_finalset_rda1_rda3.pdf"),
    width = 8,
    height = 8)
...
plink_1.9 --vcf ll_finalset_bio2_bio6_snowdays_intersect_candidate_windows_topsnps.vcf \
 ...
 --pca --out finalset_bio2_bio6_snowdays_intersect_candidate_windows_topsnps_pca
```
Copy eigen files to laptop
```{bash}
scp ebazzicalupo@genomics-a.ebd.csic.es:/home/ebazzicalupo/Selection_Eurasian_Lynx/VCF/finalset_bio2_bio6_snowdays_intersect_candidate_windows_topsnps_pca.eigen\* Documents/Selection_Eurasian_Lynx_v2/4-Downstream_Analyses/tables/
```
prepare R
```{R}
library(tidyverse)
...
pca <- read_table2("4-Downstream_Analyses/tables/finalset_bio2_bio6_snowdays_intersect_candidate_windows_topsnps_pca.eigenvec",
                   col_names = FALSE)
eigenval <- scan("4-Downstream_Analyses/tables/finalset_bio2_bio6_snowdays_intersect_candidate_windows_topsnps_pca.eigenval")
...
# plot PC1 PC2 without labels
pdf(file = paste0("4-Downstream_Analyses/plots/candidate_finalset_pca1_pca2.pdf"),
    width = 8,
    height = 8)
...
# plot PC1 PC3 without labels
pdf(file = paste0("4-Downstream_Analyses/plots/candidate_finalset_pca1_pca3.pdf"),
    width = 8,
    height = 8)
...
plink_1.9 --vcf ll_wholegenome_LyCa_ref.sorted.filter7.finalset.maf5pc.intergenic.noselection.vcf \
 --double-id --allow-extra-chr --set-missing-var-ids @:# \
 --remove samplestoremove_pca.txt --extract <(shuf -n 10000 plink.prune.in) \
 --pca --out finalset_intergenic_neutral_snps_pca
```
Copy eigen files to laptop
```{bash}
scp ebazzicalupo@genomics-a.ebd.csic.es:/home/ebazzicalupo/Selection_Eurasian_Lynx/VCF/finalset_intergenic_neutral_snps_pca.eigen\* Documents/Selection_Eurasian_Lynx_v2/4-Downstream_Analyses/tables/
```
prepare R
```{R}
library(tidyverse)
...
```{R}
# import eigen vec and val
pca <- read_table("4-Downstream_Analyses/tables/finalset_intergenic_neutral_snps_pca.eigenvec",
                   col_names = FALSE)
eigenval <- scan("4-Downstream_Analyses/tables/finalset_intergenic_neutral_snps_pca.eigenval")
...
# plot PC1 PC2 without labels
pdf(file = paste0("4-Downstream_Analyses/plots/neutral_finalset_pca1_pca2.pdf"),
    width = 8,
    height = 8)
...
# plot PC1 PC3 without labels
pdf(file = paste0("4-Downstream_Analyses/plots/neutral_finalset_pca1_pca3.pdf"),
    width = 8,
    height = 8)
